...
CACHE_FILE = "./dataset_ev_phase9c_base3.pkl"
...
# 1. データ抽出 (テンソル + メタ + 行動)
# =========================================
def extract_phase9c_base3_data():
    if os.path.exists(CACHE_FILE):
        print(f"📦 キャッシュファイル {CACHE_FILE} からデータを読み込みます...")
        with open(CACHE_FILE, 'rb') as f: return pickle.load(f)
        
    print(f"📁 {LOGS_DIR} から行動条件付きEVデータ(CNN用テンソル込)を抽出中... (目標: {TARGET_KYOKU_COUNT}局)")
        ...
        for log_id in list(set(log_ids)):
            ...
            for node in root:
                ...
                if tag == "INIT":
                    ...
                    action_feat = np.concatenate([[0.0], act_one_hot]) # 0.0 = 打牌
...
# 3. 実行 & レポート
# =========================================
records = extract_phase9c_base3_data()
...
print(f"📊 データ分布: マイナス({sum(y)}) vs 非マイナス({len(y)-sum(y)})")
...
def print_metrics(name, y_true, probs):
    ...
    print(f"  [Target=1 (マイナス) 予測] F1: {f1_score(y_true, preds, zero_division=0)*100:.1f}% (Pre: {precision_score(y_true, preds, zero_division=0)*100:.1f}%, Rec: {recall_score(y_true, preds, zero_division=0)*100:.1f}%)")

print("\n" + "="*80)
print("👑 フェーズ9C: EV予測 Base 1〜3 最終比較レポート")
print("="*80)
