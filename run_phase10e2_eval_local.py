...
class EvalAgent:
    def __init__(self, agent_id, models, params):
        ...
        self.a_base = params["a_base"]
        self.b_base = params["b_base"]
        self.a_good = params["a_good"]
        self.a_last = params["a_last"]
        self.a_dealer = params["a_dealer"]
        ...
        self.stats = {
            ...
            "last_kyoku": 0, "last_score": 0,
            "good_shape_kyoku": 0, "good_shape_agari": 0
        }
        self.reset_kyoku()
    ...
    def reset_kyoku(self):
        ...
        self.shanten = 2
        self.was_good_shape = False # その局で好形(1向聴以上)になったか
        self.stats["total_kyoku"] += 1

    def advance_shanten(self):
        if self.shanten > 0: self.shanten -= 1
        if self.shanten <= 1: self.was_good_shape = True
...
def distribute_rewards(agents, winner, loser, is_tsumo, oya_id, ranks):
    ...
    for a in agents:
        ...
        if a.was_good_shape: a.stats["good_shape_kyoku"] += 1
    ...
    if winner is not None:
        ...
        if agents[winner].was_good_shape: agents[winner].stats["good_shape_agari"] += 1
...
# 🎬 5. フルバッチ実行 (10,000局)
# =========================================
def evaluate_config(models, config_name, params, num_games=10000):
    agents = [
        EvalAgent(0, models, params),
        EvalAgent(1, models, params), 
        EvalAgent(2, models, params),
        EvalAgent(3, models, params)
    ]
...
def main():
    ...
    print(f"\n🚀 Phase 10E-2 再評価: 上位5設定の長期シミュレーション ({num_games}局)")
    
    # Optunaから抽出した上位5つのパラメータ
    configs = [
        ("Rank 1 (Trial 15)", {"a_base": 0.0256, "b_base": 0.1776, "a_good": 0.0398, "a_last": 0.0066, "a_dealer": 0.0095}),
        ("Rank 2 (Trial 40)", {"a_base": 0.0288, "b_base": 0.1711, "a_good": 0.0228, "a_last": 0.0097, "a_dealer": 0.0048}),
        ("Rank 3 (Trial 33)", {"a_base": 0.0233, "b_base": 0.1763, "a_good": 0.0368, "a_last": 0.0049, "a_dealer": 0.0049}),
        ("Rank 4 (Trial 42)", {"a_base": 0.0374, "b_base": 0.1744, "a_good": 0.0352, "a_last": 0.0072, "a_dealer": 0.0019}),
        ("Rank 5 (Trial 24)", {"a_base": 0.0216, "b_base": 0.1772, "a_good": 0.0400, "a_last": 0.0086, "a_dealer": 0.0035})
    ]
    ...
    for name, params in configs:
        print(f"🔄 評価中: {name} ...")
        stats = evaluate_config(models, name, params, num_games)
        results.append((name, stats))
    print(f"✅ 全シミュレーション完了 ({time.time() - start_time:.1f}秒)\n")

    print("👑 === Phase 10E-2 再評価: 長期KPI 詳細レポート ===")
    for name, s in results:
        tk = max(1, s["total_kyoku"])
        oya_k = max(1, s["oya_kyoku"])
        last_k = max(1, s["last_kyoku"])
        gs_k = max(1, s["good_shape_kyoku"])
        ...
        print(f"■ {name}")
        print(f"  [全体] 局収支: {avg_tot:>6.1f} | 和了率: {s['agari_count']/tk*100:4.1f}% | 放銃率: {s['houju_count']/tk*100:4.1f}% | リーチ率: {s['riichi_count']/tk*100:4.1f}% | 副露率: {s['call_count']/tk*100:4.1f}% | OVR率: {ovr_rate:4.1f}%")
        print(f"  [条件] 好形時和了率: {s['good_shape_agari']/gs_k*100:4.1f}% | 親番局収支: {s['oya_score']/oya_k:>6.1f} | ラス目局収支: {s['last_score']/last_k:>6.1f}")
        print("-" * 110)
