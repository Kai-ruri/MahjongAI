...
def objective(trial):
    ...
    agents = [RLConstrainedAgent(i, models, params) for i in range(4)]
    ...
    riichi_rate = stats["riichi_count"] / tk * 100
    call_rate = stats["call_count"] / tk * 100
    
    t_opps = max(1, stats["riichi_count"] + stats["call_count"] + stats["riichi_overrides"] + stats["call_overrides"])
    ovr_rate = (stats["riichi_overrides"] + stats["call_overrides"]) / t_opps * 100
    
    # ユーザー属性に記録
    trial.set_user_attr("houju_rate", houju_rate)
    trial.set_user_attr("agari_rate", agari_rate)
    trial.set_user_attr("riichi_rate", riichi_rate)
    trial.set_user_attr("call_rate", call_rate)
    trial.set_user_attr("ovr_rate", ovr_rate)
    ...
    if call_rate > 36.0: 
        penalty += (call_rate - 36.0) * 100
        is_violated = True
    if riichi_rate < 5.0: 
        penalty += (5.0 - riichi_rate) * 100
        is_violated = True
    if ovr_rate > 20.0: 
        penalty += (ovr_rate - 20.0) * 100
        is_violated = True
        
    trial.set_user_attr("is_violated", is_violated)
        
    if is_violated:
        # 制約違反時は極端なマイナススコアにペナルティを乗せて返す (勾配を残すため)
        return avg_score - 10000.0 - penalty
    else:
        # 健全なプレイスタイルなら純粋な局収支を返す
        return avg_score

def main():
    print("\n🚀 Phase 10E-2.5: 制約付きOptuna再探索 (5変数) を開始します...")
    print(" 🚨 制約条件: 副露率≤36%, リーチ率≥5%, OVR率≤20%")
    ...
    print("\n👑 === Phase 10E-2.5: 制約付き最適化 結果レポート ===")
    
    # 制約を満たしたTrialのみ抽出
    valid_trials = [t for t in study.trials if t.state == optuna.trial.TrialState.COMPLETE and not t.user_attrs.get("is_violated", True)]
    
    if not valid_trials:
        print("⚠️ 警告: 制約を満たす健康な設定は1つも見つかりませんでした。")
    else:
        # 制約を満たした中でのBestを取得
        best = max(valid_trials, key=lambda t: t.value)
        print(f"【健康的な Best Trial Number】: {best.number}")
        print(f"  [全体] 局収支: {best.value:>6.1f} | 和了: {best.user_attrs['agari_rate']:4.1f}% | 放銃: {best.user_attrs['houju_rate']:4.1f}%")
        print(f"  [健全性] リーチ: {best.user_attrs['riichi_rate']:4.1f}% | 副露: {best.user_attrs['call_rate']:4.1f}% | OVR率: {best.user_attrs['ovr_rate']:4.1f}%")
        
        print("\n【AIが発見した『健康で最強』のアクセルワーク】")
