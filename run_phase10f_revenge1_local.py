...
# =========================================
# 🤖 3. リベンジ・エージェント (ルーター標準搭載)
# =========================================
class RevengeAgent:
    def __init__(self, agent_id, models, config_name, use_top_defense):
        self.agent_id = agent_id
        self.models = models
        self.config_name = config_name
        self.use_top_defense = use_top_defense # ⭐️ 今回のテスト対象 (トップ目守備差分)
    ...
    def get_coeffs(self, is_oya, rank):
        ...
        # ⭐️ 新規追加: トップ目守備差分の再テスト！
        if self.use_top_defense and rank == 0:
            alpha -= 0.005; beta += 0.01
    ...
    def act_discard(self, turn_count, is_oya, rank, is_riichi_others):
        ...
        if self.shanten == 0:
            ...
            if score_r > score_d:
                ...
                if pol_riichi > pol_dama: self.stats["riichi_overrides"] += 1
        ...
        if is_fold_situation:
            # 10F-3で大成功した守備ルーターは常にON
            for i in range(14):
                if self.hand[i] == 0: logits[i] += 5.0   # 現物絶対優先
                elif self.hand[i] == 1: logits[i] += 2.0 # 準安全牌
                elif self.hand[i] == 2: logits[i] -= 2.0 # 無スジ
        ...
        if is_fold_situation:
            if chosen_danger == 2:
                if random.random() < 0.10: return "houju"
...
# 🎬 5. フルバッチ実行
# =========================================
def evaluate_config(models, config_name, use_top_defense, num_games=5000):
    agents = [
        RevengeAgent(0, models, config_name, use_top_defense),
        RevengeAgent(1, models, "Base", False), 
        RevengeAgent(2, models, "Base", False),
        RevengeAgent(3, models, "Base", False)
    ]
...
def main():
    ...
    print(f"\n🚀 Phase 10F リベンジ1: トップ目守備差分 再テスト ({num_games}局)")
    
    configs = [
        ("パターンA: 現行ベスト (トップ目差分なし)", False),
        ("パターンB: A ＋ トップ目守備差分 (Δα=-0.005, Δβ=+0.01)", True)
    ]

    results = []
    start_time = time.time()
    for name, use_top_def in configs:
        print(f"🔄 評価中: {name} ...")
        stats = evaluate_config(models, name, use_top_def, num_games)
        results.append((name, stats))
    print(f"✅ 全シミュレーション完了 ({time.time() - start_time:.1f}秒)\n")

    print("👑 === Phase 10F リベンジ1: トップ目限定 KPI 詳細レポート ===")
    for name, s in results:
        ...
        print(f"■ {name}")
        print(f"  [全体] 局収支: {avg_tot:>6.1f} | 放銃率: {s['houju_count']/tk*100:4.1f}% | OVR率: {ovr_rate:4.1f}%")
        print(f"  [トップ目限定] 局収支: {avg_top:>6.1f} | 放銃率: {s['top_houju']/top_k*100:4.1f}% | 和了率: {s['top_agari']/top_k*100:4.1f}% | リーチ率: {s['top_riichi']/top_k*100:4.1f}%")
        print("-" * 100)
