...
# 🤖 3. 安全ルーター搭載エージェント
# =========================================
class SafeRouterAgent:
    def __init__(self, agent_id, models, config_name, use_router):
        self.agent_id = agent_id
        self.models = models
        self.config_name = config_name
        self.use_router = use_router # ⭐️ 守備ルーターのON/OFF
        ...
        self.stats = {
            ...
            # 守備ルーター用KPI
            "fold_situations": 0, "genbutsu_available": 0, "safe_available": 0,
            "missed_genbutsu": 0, "missed_safe": 0,
            "cut_danger": 0, "houju_by_danger": 0
        }
        self.reset_kyoku()
    ...
    def act_discard(self, turn_count, is_oya, rank, is_riichi_others):
        ...
        if is_fold_situation:
            ...
            # 🛡️ 守備打牌再ランキング (Safety-Router 発動!)
            if self.use_router:
                for i in range(14):
                    if self.hand[i] == 0: logits[i] += 5.0   # 現物に絶対的な加点
                    elif self.hand[i] == 1: logits[i] += 2.0 # 準安全牌に加点
                    elif self.hand[i] == 2: logits[i] -= 2.0 # 無スジにペナルティ
        ...
        if is_fold_situation:
            ...
            if chosen_danger == 2:
                ...
                if random.random() < 0.10: return "houju" # 10%で無スジ放銃
            elif chosen_danger == 1 and random.random() < 0.01:
                return "houju" # 1%でスジ引っ掛け放銃
...
def run_kyoku(agents):
    ...
    for turn in range(70):
        ...
        if action == "houju":
            # ルーター内で放銃判定が出た場合 (他家のリーチ者に振り込んだ扱い)
...
# 🎬 5. フルバッチ実行
# =========================================
def evaluate_config(models, config_name, use_router, num_games=5000):
    agents = [
        SafeRouterAgent(0, models, config_name, use_router),
        SafeRouterAgent(1, models, "Base", False), 
        SafeRouterAgent(2, models, "Base", False),
        SafeRouterAgent(3, models, "Base", False)
    ]
...
def main():
    ...
    print(f"\n🚀 Phase 10F-3: 守備打牌再ランキング シミュレーション ({num_games}局)")
    
    configs = [
        ("パターンA: ルーターなし (10F-1のまま)", False),
        ("パターンB: 守備ルーターあり (現物優先バイアス)", True)
    ]

    results = []
    start_time = time.time()
    for name, use_router in configs:
        print(f"🔄 評価中: {name} ...")
        stats = evaluate_config(models, name, use_router, num_games)
        results.append((name, stats))
    print(f"✅ 全シミュレーション完了 ({time.time() - start_time:.1f}秒)\n")

    print("👑 === Phase 10F-3: 守備打牌ルーター KPI 詳細レポート ===")
    for name, s in results:
        tk = s["total_kyoku"]
        avg_tot = s["total_score"] / tk
        
        # 守備KPIの計算
        g_avail = max(1, s["genbutsu_available"])
        s_avail = max(1, s["safe_available"])
        c_danger = max(1, s["cut_danger"])
        
        print(f"■ {name}")
        print(f"  [全体] 局収支: {avg_tot:>6.1f} | 和了: {s['agari_count']/tk*100:4.1f}% | 放銃: {s['houju_count']/tk*100:4.1f}%")
        print(f"  [守備KPI] 現物見落とし率: {s['missed_genbutsu']/g_avail*100:4.1f}% | 安牌見落とし率: {s['missed_safe']/s_avail*100:4.1f}% | 危険打牌放銃率: {s['houju_by_danger']/c_danger*100:4.1f}%")
        print("-" * 100)
